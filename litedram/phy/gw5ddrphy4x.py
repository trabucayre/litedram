...
class GW5DDRPHY4X(Module, AutoCSR):
    def __init__(self, pads,
        ...
        # Init -------------------------------------------------------------------------------------
        self.submodules.init = GW5DDRPHY4XInit()
        ...
        for i in range(databits//8):
            ...
            dqs_o       = Signal()
            dqs_o_oen   = Signal()
            dqs_phys_oe = Signal(4)
            self.comb += [
                dqs_phys_oe[0].eq(~(dqs_oe | dqs_postamble)),
            ...
            self.specials += [
                Instance("OSER8_MEM",
                    ...
                    i_FCLK  = ClockSignal("sys4x"),
                    i_TCLK  = dqsw,
                    **{f"i_TX{n}": dqs_phys_oe[n] for n in range(4)}, # CHECKME: Polarity + Latency.
                    **{f"i_D{n}": (0b1010 >> n) & 0b1 for n in range(8)},
            ...
            for j in range(8*i, 8*(i+1)):
                ...
                self.specials += Instance("OSER8_MEM",
                    ...
                    i_FCLK  = ClockSignal("sys4x"),
                    i_TCLK  = dqsw270,
                    **{f"i_TX{n}": ~dq_oe for n in range(4)}, # CHECKME: Polarity
                    **{f"i_D{n}": dq_o_data_muxed[n] for n in range(8)},
                    o_Q0    = dq_o,
                    o_Q1    = dq_o_oen,
                )
                ...
                self.specials += Instance("IDES8_MEM",
                    i_RESET = ResetSignal("sys"),
                    i_PCLK  = ClockSignal("sys"),
                    i_FCLK  = ClockSignal("sys4x"),
                    i_ICLK  = dqsr90,
                    ...
                    i_D     = dq_i,
                    i_CALIB = 0,
                    **{f"o_Q{n}": dq_i_bitslip.i[n] for n in range(8)},
                )
